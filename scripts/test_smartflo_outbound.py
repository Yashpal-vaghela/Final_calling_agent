...
async def trigger_direct_call(customer_number: str):
    ...
    res = await smartflo_client.initiate_click_to_call(
        ...
        custom_params={"opening_intent": "outbound_consultation"}
    )
...
def main():
    if len(sys.argv) < 2:
        print("Usage:")
        print("  1. Open Web Form   : python scripts/test_smartflo_outbound.py <customer_number>")
        print("  2. Direct Call     : python scripts/test_smartflo_outbound.py <customer_number> --direct")
        print("Example:")
        print("  python scripts/test_smartflo_outbound.py 8758657212 --direct")
        sys.exit(1)
    ...
    print(f"  python scripts/test_smartflo_outbound.py {customer_number} --direct")
    print("=" * 65 + "\n")
