...
async def trigger_direct_call(customer_number: str):
    ...
    res = await smartflo_client.initiate_click_to_call(
        ...
        custom_params={"opening_intent": "outbound_smile_preview"}
    )
...
def main():
    if len(sys.argv) < 2:
        print("Usage:")
        print("  1. Open Web Form   : python scripts/test_smile_preview_outbound.py <customer_number>")
        print("  2. Direct Call     : python scripts/test_smile_preview_outbound.py <customer_number> --direct")
        print("Example:")
        print("  python scripts/test_smile_preview_outbound.py 8758657212 --direct")
        sys.exit(1)
    ...
    form_url = f"http://localhost:8050/smile-preview?phone={encoded_phone}"

    print("\n" + "=" * 65)
    print("    ULTIMATE SMILE DESIGN — AI SMILE PREVIEW LEAD FORM")
    print("=" * 65)
    print(f"Target Destination Phone : {customer_number}")
    print(f"Opening AI Preview Form URL : {form_url}")
    print("=" * 65)
    print("Please enter the caller's Name, Email, and City")
    print("in the browser form and click 'Talk to a Smile Designer'.")
    print("Tip: Add '--direct' flag to place direct outbound call from terminal:")
    print(f"  python scripts/test_smile_preview_outbound.py {customer_number} --direct")
    print("=" * 65 + "\n")
